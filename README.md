- [🚴‍♂️ 基础知识](#️-基础知识)
  ...
  - [javascript](#javascript)
      ...
      - [5. 什么是闭包？](#5-什么是闭包)
      - [6. v8 GC 回收有哪些方式，回收过程分别是什么？](#6-v8-gc-回收有哪些方式回收过程分别是什么)
      - [7. 函数柯理化+不定参数获取 的字节真题。](#7-函数柯理化不定参数获取-的字节真题)
  - [前端存储](#前端存储)
      - [1. 前端存储有哪些方式？cookie 跟 indexDB 有啥区别？是否持久化？](#1-前端存储有哪些方式cookie-跟-indexdb-有啥区别是否持久化)
      - [cookie 如何携带到跨域的接口上？](#cookie-如何携带到跨域的接口上)
      - [cookie如果很大有什么危害？该怎么优化？](#cookie如果很大有什么危害该怎么优化)
- [🤺 前端框架/库](#-前端框架库)
  - [React](#react)
  ...
  - [微前端](#微前端)
      ...
      - [3. 为什么选择微前端的技术架构？为什么选 qiankun？](#3-为什么选择微前端的技术架构为什么选-qiankun)
  - [低代码](#低代码)
...
- [⚔️ 工程化](#️-工程化)
      - [1. 结合过往经验，你如何理解前端工程化？](#1-结合过往经验你如何理解前端工程化)
  - [模块化](#模块化)
      - [1. 一键换肤怎么实现？切换换肤方案会刷新页面吗？如何做到不刷新页面换肤？](#1-一键换肤怎么实现切换换肤方案会刷新页面吗如何做到不刷新页面换肤)
  - [组件化](#组件化)
...
- [👑 性能优化](#-性能优化)
  - [语义化 \& SEO](#语义化--seo)
      - [1. 官网类应用 SEO 做过哪些优化措施？](#1-官网类应用-seo-做过哪些优化措施)
  - [高性能页面](#高性能页面)
      ...
      - [5. FCP、LCP、CLS是什么？](#5-fcplcpcls是什么)
  - [架构设计](#架构设计)
      ...
      - [5. 官网类应用优化前后的 qps 是多少？怎么去计算？](#5-官网类应用优化前后的-qps-是多少怎么去计算)
  - [监控预警](#监控预警)
...
- [数据结构\&算法](#数据结构算法)
      - [1. 写一个数字转千分位的函数，需考虑边界和极端情况？如 1000000 =\> 1,000,000](#1-写一个数字转千分位的函数需考虑边界和极端情况如-1000000--1000000)
      - [2. 不定位数版本号数组排序](#2-不定位数版本号数组排序)
- [🤡 兼容](#-兼容)
- [⚓️ 后端知识等](#️-后端知识等)
  - [node.js](#nodejs)
      - [1. node 的动态扩展如何实现？](#1-node-的动态扩展如何实现)
      - [2. GraphQL 使用有哪些缺点？](#2-graphql-使用有哪些缺点)
      - [3. node 调用别的借口比较慢如何去分析？](#3-node-调用别的借口比较慢如何去分析)
      - [4. 遇到过node 内存泄漏吗？如何排查定位的？最终原因是什么？](#4-遇到过node-内存泄漏吗如何排查定位的最终原因是什么)
  - [linux运维](#linux运维)
...
- [📚 项目管理](#-项目管理)
  ...
  - [团队协作](#团队协作)
      ...
      - [2. 团队管理做过哪些有效措施？怎么保障技术分享能坚持做下去？](#2-团队管理做过哪些有效措施怎么保障技术分享能坚持做下去)
  - [敏捷Scrum](#敏捷scrum)
...
1. **减少渲染次数**：React 中虚拟 DOM 的机制可以减少渲染次数，但仍需要避免不必要的重复渲染。可以使用 shouldComponentUpdate 函数进行判断，只有当 props 或 state 发生变化时才触发重新渲染。

2. **使用 React.memo**：React.memo 可以缓存组件渲染结果，当组件的 props 没有变化时，可以直接返回之前的结果，从而提高性能。

3. **利用索引键**值：在列表组件中，使用带有唯一标识符的数据结构作为 key，可以提高列表渲染性能。

4. **分割大型组件**：将一个大型组件拆分成多个小型组件，可以降低每个组件的复杂度和渲染时间，提高整体性能。

5. **延迟加载组件**：对于非常大的组件或者不常用的组件，使用延迟加载的方式，在需要时再进行加载，可以提高应用程序的启动速度。

6. **依赖懒加载**：只有在需要时才加载所需的依赖项，可以提高应用程序的加载速度和性能。

7. **使用浏览器 & http缓存**：合理地使用浏览器缓存，可以减少页面的请求次数和响应时间，从而提高应用程序的性能。

8. **使用 Web Workers**：使用 Web Workers 可以将一些计算密集型任务移动到后台线程中，从而减少主线程的压力，提高应用程序的响应速度和性能。
...
当时面试官就问我这个复杂度是多少，我回答说是 3n。然后我立马意识到还可以用一个 for 循环倒叙处理搞掂，我立马回答我觉得还有个更好的的做法实现，复杂度可以降为 n。
